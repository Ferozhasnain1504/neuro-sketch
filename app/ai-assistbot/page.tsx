...
import WaveformPanel, { Trace } from "@/components/WaveformPanel"
...
import { ExternalLink, HelpCircle, Code, Github, Mic, MicOff, Volume2, VolumeX, ArrowLeft, Home } from "lucide-react";
...
const AiAssistBotPage = () => {
  ...
  const [speechDebugInfo, setSpeechDebugInfo] = useState<string>("");
    ...
    // Waveform traces for Plotly
  const [traces, setTraces] = useState<Trace[]>([])

  // optional: control visibility of embedded CircuitJS iframe
  const [showCircuitIframe, setShowCircuitIframe] = useState(false)
  ...
  // convert currentCircuitJson into simple demo traces so the WaveformPanel shows something
  useEffect(() => {
    ...
    try {
      // Simple demo logic: for each device, create a trace.
      // If device.type === "Button" we make a square pulse, otherwise a steady value.
      const devicesObj: any = (currentCircuitJson as any).devices || {}
      const deviceKeys = Object.keys(devicesObj)
      ...
      const tSteps = 100
      const duration = 1.0 // 1 second timeline
      const dt = duration / (tSteps - 1)
      const tArr = Array.from({ length: tSteps }, (_, i) => +(i * dt).toFixed(4))

      const newTraces: Trace[] = deviceKeys.map((devId, idx) => {
        const dev = devicesObj[devId]
        const name = dev?.label || devId
        // Button -> square wave, Lamp/output -> a slower toggle, others -> small analog-ish sine
        let y: number[] = []
        if (dev?.type === 'Button') {
          // square wave: 0/1 toggling based on index
          y = tArr.map((_, i) => (Math.floor(i / 10) % 2 === 0 ? 1 : 0))
        } else if (dev?.type === 'Lamp' || dev?.type === 'Output') {
          y = tArr.map((_, i) => (Math.sin((i / tSteps) * Math.PI * 2 * (idx + 1)) > 0 ? 1 : 0))
        } else {
          // analog-ish small waveform
          y = tArr.map((tt, i) => Math.sin((i / tSteps) * Math.PI * 2 * (idx + 1)) * (1 + idx * 0.2))
        }
        return { name, x: tArr, y, mode: 'lines' }
      })

      setTraces(newTraces)
    } catch (err) {
      console.error("Error creating demo traces:", err)
      setTraces([])
    }
  }, [currentCircuitJson])
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen max-h-screen overflow-hidden">
      ...
      <div className="flex-grow flex flex-col overflow-auto p-4 md:block hidden">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-4">
            <Link href="/" className="flex items-center gap-1 text-blue-600 hover:text-blue-800 transition-colors">
              <Home size={20} />
            </Link>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setShowCircuitIframe(v => !v)}
            >
              {showCircuitIframe ? 'Hide Analog Simulator' : 'Open Analog Simulator'}
            </Button>
          </div>
        </div>
        <div className="flex-grow flex items-center justify-center">
          ...
          {/* Waveform panel — shows traces converted from currentCircuitJson */}
          <div className="w-full max-w-5xl mx-auto mt-4 md:hidden">
            <WaveformPanel traces={traces} height={220} />
          </div>
        </div>
      </div>
      ...
      {/* Hidden/dockable analog simulator (CircuitJS) */}
      <CircuitIframe
        visible={showCircuitIframe}
        onWaveformMessage={(payload) => {
          // If your hosted/modified CircuitJS posts waveform messages, they'll arrive here.
          // Example payload expected: { traces: [{name, x:[], y:[]}, ...] }
          if (payload?.traces) {
